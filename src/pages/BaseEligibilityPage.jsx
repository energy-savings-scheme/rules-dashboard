...
export default function BaseEligibility(props) {
  const { variables, entities, selected_variable } = props;
  ...
  useEffect(() => {
    if (selected_variable && variables) {
      const selected_variable_data = variables.filter((item) => offsprings.includes(item.name));
